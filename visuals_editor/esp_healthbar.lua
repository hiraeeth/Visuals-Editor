...
local menu = {
    enabled = ui.new_checkbox("Visuals", "Player ESP", "Custom Health bar"),
...
ui.set_visible(menu.gradient, ui.get(menu.enabled))
ui.set_visible(menu.label, ui.get(menu.enabled))
ui.set_visible(menu.label2, ui.get(menu.enabled))
ui.set_visible(menu.colorpicker, ui.get(menu.enabled))
ui.set_visible(menu.colorpicker2, ui.get(menu.enabled))
...
local function handle_checkbox()
    ...
    ui.set_visible(menu.gradient, enabled)
    ui.set_visible(menu.label, enabled)
    ui.set_visible(menu.label2, enabled)
    ui.set_visible(menu.colorpicker, enabled)
    ui.set_visible(menu.colorpicker2, enabled)
end

ui.set_callback(menu.enabled, handle_checkbox)
...
client.set_event_callback("paint", function()
    if ui.get(menu.enabled) then
        local r, g, b, a = ui.get(menu.colorpicker)
        local r2, g2, b2, a2 = ui.get(menu.colorpicker2)
            ...
            if entity.is_alive(local_player) or not force_teammates or force_teammates and (not entity.is_alive(local_player) and not entity.is_enemy(e)) then
                ...
                if x1 ~= nil and y1 ~= nil and not entity.is_dormant(e) then
                    ...
                    if hp ~= nil then
                        ...
                        if ui.get(menu.gradient) then
                            renderer.gradient(leftside, math.ceil(y2 - (height * percentage)) + 2, 2, math.floor(height * percentage) - 2, new_r, new_g, new_b, 255, r2, g2, b2,
                                255, false)
                        else
            ...
            for k, v in pairs(players) do
                if not v.active and entity.is_alive(v.ent) then
                    ...
                    if x1 ~= nil and y1 ~= nil then
                        ...
                        local new_r, new_g, new_b = lerp(r2, r, percentage), lerp(g2, g, percentage), lerp(b2, b, percentage)
                        if ui.get(menu.gradient) then
                            renderer.gradient(leftside, math.ceil(y2 - (height * percentage)) + 2, 2, math.floor(height * percentage) - 2, new_r, new_g, new_b, players[k].alpha,
                                r2, g2, b2, players[k].alpha, false)
                        else
